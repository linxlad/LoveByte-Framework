	...
	<div class="Container">
		...
		<div class="Content">
			...
			<?php echo Form::open('invite', 'POST', array('class' => 'formy')); ?>
				...
				echo $errors->has( 'username' ) ? $errors->first( 'username', '<span class="error">:message</span>') : ''; ?> 
				<?php } else { echo Form::label('username', 'Reserve a username:' , array('class' => 'invitelabel')); } ?><br>                
                <?php echo Form::text('username', Input::old('username'), array('class' => 'inputUsername')); ?><br><br>
                <?php if($errors->has( 'email' )) {
				echo $errors->has( 'email' ) ? $errors->first( 'email', '<span class="error">:message</span>') : ''; ?>
				<?php } else { echo Form::label('email', 'Email Address:', array('class' => 'invitelabel')); } ?><br>
                <?php echo Form::email('email', Input::old('email'), array('class' => 'inputEmail')); ?><br><br>
                <?php if($errors->has( 'language' )) {
				echo $errors->has( 'language' ) ? $errors->first( 'language', '<span class="error">:message</span>') : ''; ?> 
				<?php } else { echo Form::label('language', 'Preferred programming language(s):', array('class' => 'invitelabel')); } ?><br>
                <?php echo Form::text('language', 'e.g. C#, PHP, etc.. (Comma Separated)', array('class' => 'inputLanguage', 'onfocus' => 'value = ""')); ?><br><br>
